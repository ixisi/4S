...
ANIM_LIB["move"] = function(src, state, args)
    ...
    if not state.init then
        ...
        state.duration = parse_time(args[#args]) or 0
        ...
        -- [[ SYNC SHADOW STATE ]] --
        -- Ensure we are calculating from the latest "Virtual" position
        -- This fixes the "Jumping" issue if previous moves are still in queue
        if src.scene_item.transform then src.scene_item.transform(nil) end -- Trigger a sync if needed
        ...
        for _, pair in ipairs(safe_split(move_args)) do
            ...
            if k and v then
                ...
                -- Get Start Value (Prefer Virtual/Shadow values if available)
                local current_val = 0
                if k == "x" then
                    current_val = src.scene_item.pos().x
                elseif k == "y" then
                    current_val = src.scene_item.pos().y
                elseif k == "rot" or k == "rotation" then
                    current_val = src.scene_item.rot()
                elseif k:match("scale") then
                    current_val = src.scene_item.scale()[k:match("scale%.(%w+)")] or 1
                elseif src.scene_item[k] then
                    current_val = src.scene_item[k]() or 0
                end
    ...
    if not state.hasFinished then
        ...
        for k, target in pairs(state.targets) do
            ...
            if k == "x" then
                src.scene_item.pos({ x = val })
            elseif k == "y" then
                src.scene_item.pos({ y = val })
            elseif k == "rot" or k == "rotation" then
                src.scene_item.rot(val)
            elseif k == "scale.x" then
                src.scene_item.scale({ x = val })
            elseif k == "scale.y" then
                src.scene_item.scale({ y = val })
            elseif src.scene_item[k] then
                src.scene_item[k](val)
            end
        end
    end

    -- 4. Finish Check (UPDATED)
    if t >= 1 or state.hasFinished then
        state.hasFinished = true
        -- [[ THE FIX: WAIT FOR VISUALS TO CATCH UP ]] --
        -- Even if the math is done (t=1), we check if the object still has tasks in the queue.
        -- We peek into the wrapper's _queue. If it has items, we return FALSE (wait).
        if src.scene_item._queue and #src.scene_item._queue > 0 then
            return false
        end
        return true
    else
...
function setup()
    ...
    function _.finally(src)
        ...
        local scene_item = obs.front.source(src.source)
        ...
        if scale then
            src.defaultScaleX = scale.x; src.defaultScaleY = scale.y
        end
    ...
    function _.tick(src)
        ...
        if not src.filter or not src.isAlive or not obslua.obs_source_enabled(src.filter) then
            if not obs.utils.script_shutdown and src.transform and not src.transform_ok then
                src.transform_ok = true
                local scene_item = src.scene_item
                if not scene_item or not scene_item.data then return end
                if src.style then
                    scene_item.style.set(src.style)
                end
                scene_item.transform(src.transform)
                scene_item.free()
            end
            src.current_idx = 1
            src.state = {}
            src.async_tasks = {}
            src.current_easing = "linear"
            return
        end
        if src.transform_ok then
            src.transform_ok = false
        end
        ...
        --     end
            ...
            -- if src.current_idx > #src.queue then 
            --     src.current_idx = 1
            --     src.state = {}
            --     src.async_tasks = {}
            -- end
    ...
    function run_code(src, settings)
        ...
        print("[4IM3-SCRIPT] Compilation finished. Total lines: " .. #src.queue)
    end
end
            ...
            local obj_scene_t; obj_scene_t = {
                ...
                get = function(source_name)
                    ...
                    obj_source_t = {
                        ...
                        -- [[ CACHED C-STRUCTS (Prevent GC Thrashing) ]]
                        ...
                        -- Note: Transform still needs set_info2 because it sets everything at once.
                        ...
                        -- Filter and Style functions remain unchanged (they use source-level APIs, not sceneitem APIs)
                ...
                get_label = function(name, source)
                ...
                add_group = function(name, refresh)
                    if refresh == nil then
                        refresh = true
                    end

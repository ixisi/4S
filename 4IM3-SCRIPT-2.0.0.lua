...
function compile_line(cmd, args)
    ...
    -- Helper: Runtime Pre-Processor for String Interpolation
                ...
                if func_found then 
                    ...
                    for _, raw_arg in ipairs(runtime_args) do
                        ...
                        elseif raw_arg:match("^[%+%-%*]") then
                            -- Single Argument Relative Math (e.g. --0.05)
                            local current_val = 0
...
ANIM_LIB["then"] = function(src, state, args)
    -- Just syntax sugar, runs the command immediately
    local cmd = args[1]
    ...
    local function resolve_includes(raw_text)
        return raw_text:gsub("!include%|([^|]+)%|%+%|", function(filepath)
            ...
            if f then
                ...
                f:close()
                return resolve_includes(content) -- Recursive in case the library has includes
            end
            return "" -- If file not found, ignore
        end)
    end
    ...
    for _, step in ipairs(steps) do
        ...
        if clean_step:match("^!run%s*%{") then
            ...
            if first_brace and last_brace then
                ...
                local thread_spawner = function(ctx)
                    
                    local snapshot = {}
                    local target_vars = ctx.active_scope or {}
                    for k, v in pairs(target_vars) do
                        snapshot[k] = v
                    end
                    local new_thread = {
                        type = "THREAD",
                ...
                table.insert(queue, thread_spawner)
            end
        ...
        elseif clean_step:sub(1, 1) == "!" then
            ...
            local exe = compile_line(cmd, parts)
            if exe then table.insert(queue, exe) end
        end
    end
